...
class Database:
    ...
    def get_all_postes(self):
        ...
        cursor = conn.cursor()
        cursor.execute("""
                       SELECT id, nom_poste, utilisateur, type_poste, adresse_ip, statut, sys_exploitation
                       FROM postes
                       """)
    ...
    def update_poste(self, id, poste):
        conn = self.connect()
        cursor = conn.cursor()
        cursor.execute("""
        ...
        """, (poste.nom_poste, poste.utilisateur, poste.type_poste, poste.adresse_ip, poste.statut, poste.sys_exploitation, id))
        conn.commit()
        conn.close()
